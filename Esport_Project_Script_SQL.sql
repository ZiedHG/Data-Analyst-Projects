...
FROM  	new_schema.generalesportdata esp 	
		JOIN (	SELECT 	Game,
				...
				GROUP BY Game
				ORDER BY AverageViewers DESC) as twig
		ON esp.Game = twig.Game

-- Joined data grouped by genre
SELECT 	esp.Genre,
...
-- Data grouped game realease year
SELECT 	ReleaseDate,
		SUM(TotalEarnings) as TotalEarnings ,
		SUM(OnlineEarnings) as OnlineEarnings,
        SUM(TotalPlayers) as TotalPlayers,
...
-- Essential Data grouped by year
